    ...
    def __call__(self, epoch_id):
        ...
        while trail < self.max_trails or total_step < self.max_total_steps:
            ...
            if(pred_state_dist is not None):
                trail_obs_loss += -numpy.log(pred_state_dist[int(previous_state)].item())#TODO:不需要这个世界模型
...
class MultiAgentGenerator(OmniRLGenerator):
    ...
    def __call__(self, epoch_id):
        ...
        interactive_prompt = numpy.array([10.0, 0.0, 0.0])  # opt3 with gamma 0.994
        self.interactive_tag = numpy.array([3])  # Unknown, let model decide current policy quality
        ...
        while trail < self.max_trails or total_step < self.max_total_steps:
            ...
            # Reset environment, updated for Mujoco 3.3.1
            previous_state = self.env.reset()  # Updated syntax for Mujoco environments
            trail_state_arr.append(previous_state)
            obs_arr.append(previous_state)
            if pred_state_dist is not None:
            ...
            while not done:
                ...
                new_state, new_reward, terminated, truncated, *_ = self.env.step(action)
                done = terminated or truncated  # Updated termination logic for Mujoco 3.3.1
                ...
                # Collect gif frames
                if self.config.save_gif and trail % self.config.save_gif_gap == 0:
                    frame = self.env.render(mode='rgb_array')  # Updated rendering for Mujoco 3.3.1
                    frames.append(frame)
                ...
                if hasattr(self.config, 'save_cache') and self.config.save_cache and (total_step + step) % self.config.save_cache_gap == 0:
                    epoch_dir = os.path.join(self.config.save_cache_path, str(epoch_id))
                    os.makedirs(epoch_dir, exist_ok=True)
                    numpy.save(os.path.join(epoch_dir, f"cache_{total_step + step}.npy"), cache)
            ...
            trail += 1
            total_step += step
            self.logger(trail, total_step, step_trail[-1], rew_stat[-1], state_error[-1], reward_error[-1], success_rate[-1])
